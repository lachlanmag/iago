...
- Any workflow that changes `status` in `$REPO_ROOT/data/applications.yaml`
...
| File | Purpose |
|------|---------|
| `$REPO_ROOT/data/applications.yaml` | Pipeline tracker (read + write) |
| `$REPO_ROOT/data/config.yaml` | Optional defaults |
...
Update `$REPO_ROOT/data/applications.yaml`:
