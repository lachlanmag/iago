...
| File | Purpose |
|------|---------|
| `$REPO_ROOT/data/config.yaml` | Role priority, location rules, industry/resume fit rubric, listing freshness |
| `$REPO_ROOT/data/applications.yaml` | Pipeline tracker |
| `$REPO_ROOT/data/seen-jobs.yaml` | Canonical URLs, `listing_verified`, closing hints |
| `profile.resume_path` in config | Resume for fit re-scoring |
| Latest `$REPO_ROOT/data/daily-runs/*.md` | Optional context from most recent daily run |

Repo root is `$REPO_ROOT`. Prefix paths when workspace root ≠ `REPO_ROOT`.
...
- Read `$REPO_ROOT/data/config.yaml`, `$REPO_ROOT/data/applications.yaml`, `$REPO_ROOT/data/seen-jobs.yaml`, and resume at `profile.resume_path`.
...
`$REPO_ROOT/data/pipeline-reviews/YYYY-MM-DD.md`
...
If user confirms promotions in chat, apply updates to `$REPO_ROOT/data/applications.yaml` immediately. For each row promoted to `shortlisted`, **automatically run `company-research`** in the same session (role brief artifact + tracker `company_research` path).
...
- Reminder: promoting to `shortlisted` chains `company-research` (saves JD to `$REPO_ROOT/data/jds/` + role brief)
